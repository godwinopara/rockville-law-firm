...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-brand-blue/20 bg-brand-surface text-ink">
    ...
    {open && <nav id="mobile-menu" className="fixed inset-x-0 bottom-0 top-[79px] overflow-y-auto bg-brand-surface px-5 pb-8 pt-5 text-ink lg:hidden" aria-label="Mobile navigation"><div className="page-shell">{primaryNavigation.map((item, index) => <Link onClick={() => setOpen(false)} className="display flex items-center border-b border-brand-blue/20 py-5 text-4xl" key={item.href} href={item.href}><span className="mr-4 font-mono text-[.6rem] text-brand-blue">0{index + 1}</span>{item.label}</Link>)}<Link onClick={() => setOpen(false)} className="btn-primary mt-8" href="/contact">Book consultation<ArrowUpRight size={15} /></Link></div></nav>}
  </header>;
}
