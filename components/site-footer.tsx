  ...
  return <footer className="overflow-hidden border-t border-paper/10 bg-ink text-paper">
    <div className="page-shell pt-16 md:pt-24">
      <div className="grid gap-14 border-b border-paper/15 pb-16 lg:grid-cols-[1.05fr_.45fr_1.35fr] lg:gap-16">
        <div>
          <p className="eyebrow gold-rule text-paper/55">Rockville Legal Practitioners</p>
          ...
          <address className="mt-12 not-italic text-sm leading-7 text-paper/65"><a className="transition-colors hover:text-gold" href={`mailto:${contactDetails.email}`}>{contactDetails.email}</a><br /><a className="transition-colors hover:text-gold" href="tel:+2348033064300">{contactDetails.phonePrimary}</a><br /><a className="transition-colors hover:text-gold" href="tel:+2348034616281">{contactDetails.phoneSecondary}</a><p className="mt-4 max-w-64">{contactDetails.address}</p></address>
        </div>

        <nav aria-label="Footer navigation"><p className="eyebrow text-gold">Navigate</p><ul className="mt-6 space-y-3 text-sm text-paper/68">{primaryNavigation.map((item) => <li key={item.href}><Link className="inline-flex items-center gap-2 transition-colors hover:text-gold" href={item.href}>{item.label}<ArrowUpRight aria-hidden="true" size={12} /></Link></li>)}</ul></nav>

        <nav aria-label="Practice areas"><p className="eyebrow text-gold">Practice areas</p><ol className="mt-6 grid gap-x-8 gap-y-3 text-sm text-paper/68 sm:grid-cols-2">{practiceAreas.map((area) => <li key={area.slug}><Link className="group grid grid-cols-[2rem_1fr] gap-2 transition-colors hover:text-gold" href={`/services/${area.slug}`}><span className="font-mono text-[.65rem] text-paper/35 group-hover:text-gold">{area.number}</span><span>{area.title}</span></Link></li>)}</ol></nav>
      </div>
