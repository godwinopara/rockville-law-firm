...
import { TeamPortraitFrame } from "@/components/team-portrait-frame";
...
export function TeamSection() {
  return <section className="bg-[#fdfcf9]" aria-labelledby="team-heading"><div className="page-shell section-space">
    ...
    <div className="mt-16 grid border-t border-line md:grid-cols-3">{teamMembers.map((member, index) => <article data-team-member-card key={member.name} className={`border-b border-line py-9 md:px-8 md:py-12 ${index > 0 ? "md:border-l" : "md:pl-0"}`}><MotionReveal><TeamPortraitFrame member={member} variant="card" /><span className="mt-8 block font-mono text-xs text-brand-blue">0{index + 1}</span><h3 className="display mt-5 text-3xl leading-[.98]">{member.name}</h3><p className="mt-3 text-sm uppercase tracking-[.13em] text-brand-blue">{member.role}</p><p className="mt-7 leading-7 text-stone">{member.summary}</p></MotionReveal></article>)}</div>
  </div></section>;
}
