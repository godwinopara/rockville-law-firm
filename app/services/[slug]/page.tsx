...
export default async function ServiceDetailPage({ params }: ServicePageProps) {
  ...
  return <>
    ...
    <section data-service-detail-layout="editorial-sidebar" className="bg-paper"><div className="page-shell section-space grid gap-12 lg:grid-cols-[25%_15%_60%] lg:gap-0">
      <aside data-service-detail-sidebar className="self-start lg:sticky lg:top-28"><div className="border-t border-line pt-8"><div className="relative aspect-[4/5] overflow-hidden"><Image src="/images/rufus-okoli.jpg" alt="Rufus Okoli, Founder and Principal Partner" fill sizes="(min-width: 1024px) 25vw, 100vw" className="object-cover" /></div><p className="display mt-6 text-2xl">Rufus C. Okoli</p><p className="mt-1 text-sm text-stone">Founder &amp; Principal Partner</p><address className="mt-6 space-y-3 not-italic text-sm text-stone"><a className="flex items-center gap-3 hover:text-brand-blue" href={`mailto:${contactDetails.email}`}><Mail aria-hidden="true" size={16} />{contactDetails.email}</a><a className="flex items-center gap-3 hover:text-brand-blue" href="tel:+2348033064300"><Phone aria-hidden="true" size={16} />{contactDetails.phonePrimary}</a></address></div>
      </aside>
      <div data-service-detail-spacer aria-hidden="true" className="hidden lg:block" />
      <article data-service-detail-content className="min-w-0"><MotionReveal><p className="eyebrow brand-rule text-stone">Understanding the matter</p><h2 className="display mt-7 max-w-3xl text-3xl leading-[.98] md:text-4xl">Clear perspective before decisive action.</h2><p className="mt-8 max-w-3xl text-lg leading-8 text-stone">{area.overview}</p></MotionReveal>
        <MotionReveal delay={.08} className="mt-16 border-t border-line pt-12"><p className="eyebrow brand-rule text-stone">Scope of work</p><h2 className="display mt-7 text-3xl leading-[.98] md:text-4xl">How we can assist</h2><p className="mt-6 max-w-2xl leading-7 text-stone">Focused legal support shaped around the documents, decisions, and interests involved in your matter.</p><ol className="mt-10 border-t border-line">{area.matters.map((matter, index) => <li key={matter} className="grid gap-4 border-b border-line py-6 sm:grid-cols-[3rem_1fr_auto] sm:items-center"><span className="font-mono text-xs text-brand-blue">0{index + 1}</span><span className="display text-xl md:text-2xl">{matter}</span><ArrowUpRight aria-hidden="true" className="hidden text-stone sm:block" size={20} strokeWidth={1.25} /></li>)}</ol></MotionReveal>
        <MotionReveal delay={.16} className="mt-16 border-t border-line pt-12"><p className="eyebrow brand-rule text-stone">Our approach</p><h2 className="display mt-7 max-w-3xl text-3xl leading-[.98] md:text-4xl">Advice grounded in context, clarity, and careful judgment.</h2><p className="mt-8 max-w-3xl text-lg leading-8 text-stone">{area.approach}</p></MotionReveal>
      </article>
    </div></section>
