...
export default async function ServiceDetailPage({ params }: ServicePageProps) {
  ...
  return <>
    ...
    <section className="bg-paper"><div className="page-shell section-space grid gap-12 lg:grid-cols-[.55fr_1.45fr] lg:gap-24">
      <MotionReveal><p className="eyebrow brand-rule text-stone">Understanding the matter</p></MotionReveal>
    ...
    <section className="border-y border-line bg-panel"><div className="page-shell section-space">
      <MotionReveal className="grid gap-8 lg:grid-cols-2"><div><p className="eyebrow brand-rule text-stone">Scope of work</p><h2 className="display mt-7 text-5xl leading-none md:text-7xl">How we can assist</h2></div><p className="max-w-md self-end leading-7 text-stone">Focused legal support shaped around the documents, decisions, and interests involved in your matter.</p></MotionReveal>
      <ol className="mt-16 border-t border-line">{area.matters.map((matter, index) => <li key={matter}><MotionReveal className="grid gap-4 border-b border-line py-7 sm:grid-cols-[5rem_1fr_auto] sm:items-center md:py-9"><span className="font-mono text-xs text-brand-blue">0{index + 1}</span><span className="display text-3xl md:text-4xl">{matter}</span><ArrowUpRight aria-hidden="true" className="hidden text-stone sm:block" strokeWidth={1.25} /></MotionReveal></li>)}</ol>
    </div></section>

    <section className="bg-ink text-paper"><div className="page-shell section-space grid gap-14 lg:grid-cols-[.7fr_1.3fr] lg:gap-24">
      <MotionReveal><p className="eyebrow brand-rule text-paper/55">Our approach</p><span aria-hidden="true" className="display mt-10 block text-[7rem] leading-none text-brand-blue-light">“</span></MotionReveal>
